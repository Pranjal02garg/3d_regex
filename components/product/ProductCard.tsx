...
export default function ProductCard({
  ...
  return (
    <article className="group relative flex flex-col w-full h-full bg-white border border-gray-200 hover:border-[var(--ochre)] rounded-2xl p-4 sm:p-5 shadow-xs hover:shadow-md transition-all duration-300">
      <Link
      ...
      >
        ...
        <div className="flex items-center justify-between gap-2 mb-3">
          <span className="font-deva text-xs sm:text-sm font-bold text-[var(--ochre)] px-3 py-0.5 rounded-full bg-[var(--surface-2)] border border-gray-200">
            {product.devanagari}
          </span>
          <span className="eyebrow text-[10px] sm:text-xs text-gray-500 font-bold">
            {product.form.toUpperCase()} · {product.unitsPerPack} TAB
          </span>
        </div>

        {/* Official Product Cover Bottle Image Box */}
        <div className="relative flex aspect-[4/3] min-h-[220px] sm:min-h-[240px] w-full items-center justify-center overflow-hidden rounded-xl mb-4 bg-white border border-gray-100 p-3 transition-transform duration-300 group-hover:scale-[1.02]">
          <Image
            ...
            className="max-h-full max-w-full h-auto w-auto object-contain drop-shadow-md transition-transform duration-300 group-hover:scale-105"
          />
      ...
      <div className="pt-3 border-t border-gray-100 flex items-center justify-between gap-2">
        ...
        <Link
          href={`/products/${product.slug}`}
          className="bg-[#111315] text-white px-4 py-2 rounded-full text-xs font-mono font-bold hover:bg-[var(--ochre)] hover:text-black transition-colors flex items-center gap-1 cursor-pointer"
        >
          <span>VIEW</span>
          <ArrowRight size={14} />
        </Link>
